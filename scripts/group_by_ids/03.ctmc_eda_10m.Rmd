...
df<-df %>% rename(id=num) #trans is an object in a later function so better to change
```
...
- z: the use category 
- the crw (correlated random walk parameter that deals with the movement capacity) 

## 1. Model 1: z ~ habitat + crw
...
- add lasso shrinkage to the model 
